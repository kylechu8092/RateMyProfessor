...
export async function POST(req) {
  try {
    ...
    const pc = new Pinecone({
      apiKey: process.env.PINECONE_API_KEY,  // Securely using environment variable
    });
    const index = pc.index('rag').namespace('ns1');

    const openai = new OpenAI({
      apiKey: process.env.OPENAI_API_KEY, // Securely using environment variable
    });

    // Step 5: Process the user’s query
    const text = data[data.length - 1].content;
    const embeddingResponse = await openai.embeddings.create({
      model: 'text-embedding-ada-002',  // Make sure model name is correct
      input: text,
